...
export function FontSelector({ value = 'Inter', onChange, className = '' }: FontSelectorProps) {
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      <Label className="text-lavender-100">Font Family</Label>
      ...
      <div className="relative">
        ...
        <button
          ...
          className="w-full px-4 py-3 bg-navy-900 border border-navy-600 rounded-md text-left text-lavender-50 hover:border-lavender-500 focus:outline-none focus:ring-2 focus:ring-lavender-500 focus:border-lavender-500 transition-colors"
          style={{ fontFamily: selectedOption.name }}
        >
          <span className="block truncate">{selectedOption.displayName}</span>
          <span className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
            <svg
              className={`w-5 h-5 text-lavender-300 transition-transform ${isOpen ? 'rotate-180' : ''}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        {isOpen && (
          <>
            <div className="absolute z-10 w-full mt-1 bg-navy-800 border border-navy-600 rounded-md shadow-lg max-h-96 overflow-auto">
                  ...
                  <button
                    ...
                    className={`w-full px-4 py-3 text-left transition-colors ${
                      option.name === selectedFont
                        ...
                        : 'text-lavender-100 hover:bg-navy-700'
                    }`}
      ...
      {/* Live preview */}
      <div
        className="p-4 bg-navy-900 border border-navy-600 rounded-md"
        style={{ fontFamily: selectedOption.name }}
      >
        <p className="text-lavender-100 text-lg mb-2">Preview:</p>
        <h3 className="text-2xl font-semibold text-lavender-50 mb-1">
