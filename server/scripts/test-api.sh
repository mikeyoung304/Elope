...
echo "✓ Tenant A key: $TENANT_A_KEY"
echo "✓ Tenant B key: $TENANT_B_KEY"
echo ""
...
if echo "$PACKAGES_A" | grep -q "Test Package A"; then
  ...
  echo "❌ FAIL: Tenant A missing 'Test Package A'"
  exit 1
fi
...
if ! echo "$PACKAGES_B" | grep -q "Test Package A"; then
  ...
  echo "❌ FAIL: Tenant B can see 'Test Package A' (data leakage!)"
  exit 1
fi
...
if echo "$PACKAGES_B" | grep -q "Test Package B"; then
  ...
  echo "❌ FAIL: Tenant B missing 'Test Package B'"
  exit 1
fi
...
if ! echo "$PACKAGES_A" | grep -q "Test Package B"; then
  ...
  echo "❌ FAIL: Tenant A can see 'Test Package B' (data leakage!)"
  exit 1
fi
echo ""
