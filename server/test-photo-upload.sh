...
main() {
    ...
    if [ "$HTTP_CODE" == "201" ]; then
        FILENAME=$(echo "$BODY" | node -e "const data = JSON.parse(require('fs').readFileSync(0)); console.log(data.filename || 'unknown')")
        URL=$(echo "$BODY" | node -e "const data = JSON.parse(require('fs').readFileSync(0)); console.log(data.url || 'unknown')")
        SIZE=$(echo "$BODY" | node -e "const data = JSON.parse(require('fs').readFileSync(0)); console.log(data.size || 0)")
        ORDER=$(echo "$BODY" | node -e "const data = JSON.parse(require('fs').readFileSync(0)); console.log(data.order || -1)")
    ...
    if [ "$HTTP_CODE" == "201" ] && [ -n "$FILENAME" ] && [ "$FILENAME" != "unknown" ] && \
       [ -n "$URL" ] && [ "$URL" != "unknown" ] && \
       [ "$SIZE" != "0" ] && [ "$ORDER" != "-1" ]; then
        record_test "Verify photo metadata" "PASS" "All metadata fields present and valid"
    else
        record_test "Verify photo metadata" "FAIL" "Missing or invalid metadata fields"
    fi
    echo ""
